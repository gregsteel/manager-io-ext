...
Mirrors manager-mcp's src/manager_mcp/http_auth.py exactly (same
AllowlistedGoogleProvider approach) — this server is reachable from the open
internet and must only ever admit Cowork, so GoogleProvider (fastmcp>=2.0,
implements the MCP OAuth spec) is wrapped with an email allowlist rather than
trusting any Google account that completes login.

This OAuth client (GMAIL_RELAY_OAUTH_GOOGLE_CLIENT_ID/SECRET) is unrelated to
credentials.py's Gmail API credentials — this one gates who may connect to
this server at all; that one is this server's own Gmail + Drive API access.
"""
...
TRANSPORT_ENV = "GMAIL_RELAY_TRANSPORT"
HOST_ENV = "GMAIL_RELAY_HTTP_HOST"
PORT_ENV = "GMAIL_RELAY_HTTP_PORT"
CLIENT_ID_ENV = "GMAIL_RELAY_OAUTH_GOOGLE_CLIENT_ID"
CLIENT_SECRET_ENV = "GMAIL_RELAY_OAUTH_GOOGLE_CLIENT_SECRET"
BASE_URL_ENV = "GMAIL_RELAY_OAUTH_BASE_URL"
ALLOWED_EMAILS_ENV = "GMAIL_RELAY_ALLOWED_EMAILS"
AUTH_SESSION_TIMEOUT_SECONDS = 365 * 24 * 60 * 60


class TransportConfigError(ValueError):
    """Invalid GMAIL_RELAY_TRANSPORT / OAuth environment configuration."""
...
def build_run_kwargs(environ: dict[str, str] | None = None) -> dict[str, Any]:
    ...
    Empty dict (stdio, fastmcp's default) unless GMAIL_RELAY_TRANSPORT=http.
    """
